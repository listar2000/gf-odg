...
import json
import os
sys.path.append("~/FoR/BlocksWorld")
...
class BlocksWorldGFNTask(LightningModule):
    def __init__(
        ...
        os.makedirs(f"GFlowPlan/transitions/{args.step}", exist_ok=True)
    ...
    def test_step(self, problem, batch_idx):
        ...
        with open(f'GFlowPlan/success_plans_test/8_step/success_text_{batch_idx}.csv', mode='w', newline='', encoding='utf-8') as file:
        ...
        for _ in range(20):
            ...
            if sum(meetings) == len(meetings):
                ...
                if (GOAL, INIT, actions_joined) not in success_text:
                    total_solution += 1
                    success_text.append((GOAL, INIT, actions_joined))
        with open(f'GFlowPlan/success_plans_valid/{self.args.step}_step/success_text_{batch_idx}.csv', mode='w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            # 写入列名
            writer.writerow(['Goal', "Initial State", 'Generated plan'])
            # 写入数据
    ...
    def on_train_epoch_start(self):
        ...
        transition_path = f"GFlowPlan/transitions/{self.args.step}/transition.pkl"
