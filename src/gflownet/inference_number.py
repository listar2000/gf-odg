...
def extract_numbers_from_concepts(sequences, text_processor):
    """
    Extract numbers from ConceptBlocks in generated sequences.
    """
    extracted_numbers = []
    for sequence in sequences:
        decoded_texts = [text_processor.tokenizer.decode(token, skip_special_tokens=True) for token in sequence]
...
def run_inference(model, tokenizer, gen_config, text_processor, N, output_csv):
    ...
    for i in range(N // gen_config.batch_size):  # Loop through in batches
        ...
        extracted_numbers = extract_numbers_from_concepts(generations["sequences"], text_processor)

        for j in range(gen_config.batch_size):
            results.append([i * gen_config.batch_size + j + 1, extracted_numbers[j]])
            print(f"\nRun {i * gen_config.batch_size + j + 1}: {extracted_numbers[j]}")

    # Save results to CSV
    with open(output_csv, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Run", "Extracted Numbers"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_csv", type=str, default="inference_results.csv", help="CSV file to save results")

    args = parser.parse_args()

    # Load model using existing `get_lora_model`
    model, tokenizer, sentence_transformer, _ = get_lora_model(
    ...
    # Run inference
    run_inference(model, tokenizer, gen_config, text_processor, args.N, args.output_csv)
